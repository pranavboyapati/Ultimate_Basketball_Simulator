...
public class WariorsClicked : MonoBehaviour
{
    //initialize variables
    public int coins;
    public string WariorsOwned;
    ...
    {
        coins = GetInt("Coins");
        WariorsOwned = GetString("WariorsOwned");

        if (WariorsOwned == "True")
        {
            SetString("SelectedTeam", "Wariors");
        }

        if (coins >= 8000)
        {
            if (WariorsOwned == "False")
            {
                coins -= 8000;
                SetInt("Coins", coins);
                SetString("WariorsOwned", "True");
            }
        }
        else
        {
            if (WariorsOwned == "False")
            {
                SetString("NotEnoughCoinsForWariors", "True");
            }
        }
    }
